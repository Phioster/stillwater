...
`tests/run_tests.gd`. Stand: 119 Tests, 0 fehlgeschlagen.
...
| `core/` | Simulationskern, reines GDScript ohne Node-Bezug: `fishing_sim.gd`, `offline_sim.gd`, `economy.gd`, `progression.gd`, `fish_roll.gd`, `inventory.gd`, `journal.gd`, `sim_context.gd`, `still_rng.gd`, `caught_fish.gd` |
...
| `tools/` | `godot.sh` (Wrapper), `gen_class_cache.py` (Script-Class-Cache), `build_data.gd` (Dateninhalte), `gen_sprites.gd` (Sprites), `palette.gd` (Farbpalette) |
| `tests/` | `run_tests.gd` (Runner) und 16 `test_*.gd`-Suiten |
