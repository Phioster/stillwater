...
func test_add_until_full() -> void:
	...
	inv.capacity = 3
	for i in 3:
		assert_true(inv.add(CaughtFish.make(&"a", 1.0, false)))
	assert_true(inv.is_full())
	assert_false(inv.add(CaughtFish.make(&"a", 1.0, false)), "voll heißt: nichts geht mehr rein")
	assert_eq(inv.fish.size(), 3)

func test_favorites_are_not_sellable() -> void:
	var inv := Inventory.new()
	inv.capacity = 10
	var keeper := CaughtFish.make(&"a", 1.0, false)
	keeper.is_favorite = true
	inv.add(keeper)
	inv.add(CaughtFish.make(&"b", 1.0, false))
...
func test_array_roundtrip() -> void:
	var inv := Inventory.new()
	inv.capacity = 10
	inv.add(CaughtFish.make(&"bluegill", 0.3, true))
