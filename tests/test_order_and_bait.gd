...
func test_the_secret_tab_lists_zones_in_the_same_order() -> void:
	...
	var m := _main()
	m.show_tab(m.JOURNAL_TAB)
	m.get_node("SidePanel/Panels/JournalGroup").select_sub(m.JOURNAL_SUB_SECRET)
	var names: Array[String] = []
	for z in Database.zones_in_order():
		names.append(z.display_name)
	var seen := _positions(_texts(m.get_node("SidePanel/Panels/JournalGroup/SecretScroll/SecretPanel")), names)
